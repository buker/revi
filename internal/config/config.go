...
type Config struct {
	...
	Claude ClaudeConfig `mapstructure:"claude"` // Claude CLI settings
}
...
// ClaudeConfig holds configuration for the Claude CLI integration.
type ClaudeConfig struct {
	Path    string `mapstructure:"path"`    // Path to the Claude CLI executable
	Timeout int    `mapstructure:"timeout"` // Timeout in seconds for Claude requests
}
...
func setDefaults() {
	...
	viper.SetDefault("claude.timeout", 60)
}
...
// This enables flags like --review and --block to override config file settings.
