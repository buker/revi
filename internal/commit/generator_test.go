...
func TestValidateMessage(t *testing.T) {
	t.Run("valid message", func(t *testing.T) {
		err := ValidateMessage(&claude.CommitMessage{Type: "feat", Subject: "add staging diff support"})
		if err != nil {
	...
	t.Run("missing type", func(t *testing.T) {
		err := ValidateMessage(&claude.CommitMessage{Subject: "add thing"})
		if err == nil {
	...
	t.Run("invalid type", func(t *testing.T) {
		err := ValidateMessage(&claude.CommitMessage{Type: "feature", Subject: "add thing"})
		if err == nil {
			t.Fatal("expected error")
		}
	})
	...
	t.Run("missing subject", func(t *testing.T) {
		err := ValidateMessage(&claude.CommitMessage{Type: "fix"})
		if err == nil {
			t.Fatal("expected error")
		}
	})
	...
	t.Run("subject too long", func(t *testing.T) {
		...
		err := ValidateMessage(&claude.CommitMessage{Type: "chore", Subject: long})
		if err == nil {
			t.Fatal("expected error")
		}
	})
...
func TestFormatMessage(t *testing.T) {
	msg := &claude.CommitMessage{Type: "feat", Scope: "cli", Subject: "add config subcommand", Body: "Provides config show/path."}
